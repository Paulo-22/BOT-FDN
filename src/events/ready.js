...
async function configurarPaineis(client) {
  ...
  const pares = [
    {
      canalId: config.canais.registro,
      painel: paineis.painelRegistro(),
    },
    {
      canalId: config.canais.recrutamento,
    ...
    {
      canalId: config.canais.ausencias,
      painel: paineis.painelAusencias(),
    },
    {
      canalId: config.canais.punicao,
      painel: paineis.painelPunicao(),
    },
    {
      canalId: config.canais.adminPanel,
      painel: paineis.painelAdmin(),
    },
    {
      canalId: config.canais.tickets,
      painel: paineis.painelTickets(),
    },
  ];

  for (const { canalId, painel } of pares) {

    if (!canalId || canalId.startsWith('ID_')) {
      continue;
    }
    ...
    if (!canal) {
      console.warn(`⚠️ Canal não encontrado: ${canalId}`);
      continue;
    }
